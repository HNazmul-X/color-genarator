...
const colorCodeWord = ["a", "8", "f", "8", "b", "8", "f", "8", "c", "8", "f", "8", "d", "8", "f", "e", "8", "f", "8", "1", "8", "f", "8", "2", "f", "3", "f", "4", "f", "5", "f", "6", "f", "7", "f", "8", "f", "0", "f", "9"];
const addColorToArry = document.querySelector(".add-color-to-cart");
const currentColorCodeShower = document.querySelector("#color-code-shower");
let oneColor, towColor, threeColor;
// selected color arry
let selectedColor = [];
let recentlyGenarated = [];
...
const setColor = () => {
      ...
      getDataFromApi();
      currentColorCodeShower.innerText = ` ${oneColor}`;
...
const getSelectedColor = () => {
      addColorToArry.addEventListener("click", (e) => {
            if (selectedColor.indexOf(oneColor) !== -1) {
                  showingMyAlert("your already added it");
            } else {
            ...
            selectedColor.forEach((colorbox) => {
                  ...
                  div.innerHTML = `
              <i id="${colorbox}" onclick="deleteItemFromCart('${colorbox}')" class="times-icon fas fa-times"></i>
              `;
...
const deleteItemFromCart = (deleteItem) => {
      const newitem = selectedColor.filter((item) => item !== deleteItem);
...
const getDataFromApi = () => {
      const url = `https://www.thecolorapi.com/id?hex=${oneColor.replace(/#/g, "")}`;
      fetch(url)
            .then((res) => res.json())
            .then((data) => {
                  recentlyGenarated.indexOf(data) == -1 ? recentlyGenarated.push(data) : showingMyAlert("already added this color");
            });
};

// push current object to arry

/* ----------------------------------------------------
-------------  MY Alert funtion --------------------
------------------------------------------------------ */

const showingMyAlert = (title, desc) => {
      const alertBodyCover = document.querySelector(".my-alert-cover");
      alertBodyCover.style.display = "block";
      alertBodyCover.classList.add("my-alert-cover");
      alertBodyCover.querySelector(".title").innerHTML = title;
      alertBodyCover.addEventListener("click", () => {
            alertBodyCover.style.display = "none";
      });
};
